...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      if (this.roleId == 1) {
        this.$store.dispatch('admin/getCourseTopN', data)
        .then(res => {
          if (res.code === 200) {
            console.log(res);
            this.CourseTopN = res.data;
            let value = [];
            this.CourseTopN.forEach(item => {
              value.push({
                xValue: item.visCnt,
                yValue: item.courseName
              })
            })
            value.sort((a, b) => {
        ...
        this.$store.dispatch('admin/getMyCourseTopN', data)
        .then(res => {
          if (res.code === 200) {
            console.log(res);
            this.CourseTopN = res.data;
